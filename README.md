...
- **Phase 5** ✅ — landing refresh (Phase 5 badge, GitHub links, animated preview frame), `/about` + `/docs` aligned with live adapters, dismissible global disclaimer with restore from settings/home footer, postsynaptic D1–D5 binding edges + dashboard rows, TH-fully-blocked info alert, flux particles rendered inside React Flow for correct coordinates, global keyframes for motion-safe animation. HMDB bulk-import still planned.
